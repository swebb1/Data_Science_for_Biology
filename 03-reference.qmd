---
title: "Genome references and annotations"
---
...
In this section, we will use some of the Bioconductor annotation packages to fetch genomic sequences and annotations. We will also connect to external databases such as Ensembl using the *biomaRt* package and the *AnnotationHub* to download datasets from multiple sources.

## Fetching data from annotation packages and external databases
...
gr2.seq
```
The output is a DNAStringSet object which contains the sequences for each of the GRanges in gr2. We can use the functions in Biostrings to work with these sequences. For example, we can calculate the dinucleotide frequencies for each sequence.
...
head(anno)
```
We now have the Entrez gene IDs, gene symbols and Uniprot IDs for all of the Ensembl gene IDs in our GRanges object.
...
keytypes(txdb)
```
Lets get the first 10 entries in `txdb`.
...
select(txdb, keys = head(keys(txdb)),
       ...
       keytype="GENEID")
```
This shows individual transcript coordinates and the gene they belong to. The *org.db* and *TxDB* databases are all centred upon Entrez gene IDs. However, we commonly use gene annotations from the Ensembl database. It is possible to make your own txdb objects.

It is possible to build a txdb object from the Ensembl database using the `makeTxDbFromEnsembl()` function. However, this requires an additional installation of database package to connect to the Ensembl MySQL database and is not recommended for this workshop.
...
Instead, we can build a txdb object from a GTF file downloaded from Ensembl.

```{r}
## Create a txdb from a GFF file
library(txdbmaker)
txdb.gff <- makeTxDbFromGFF(file = "data/Homo_sapiens.GRCh37.74.edited.gtf",
                          format = "gtf",
...
## Fetching data from websites (biomaRt)
...
By default `useMart()` will always select the most recent release of the Ensembl database and therefore the current genome assembly, in this case hg38. Because we have so far used data from the previous assembly (hg19/GRCh37) we must select an archived version of Ensembl. The archived versions are listed [here](https://www.ensembl.org/info/website/archives/index.html).

```{r}
##use an archived version of biomart
hg19.ensembl <- useMart(biomart = "ensembl",
...
We can now query biomart using `getBM()`. Let's extract annotations for the gene *MeCP2*.
...
We now have a GRanges object containing all of the hg19 genes and transcripts annotated by Ensembl. If we wanted to, we could also create a *txdb* object with this GRanges object using `makeTxDbFromGRanges()`. 

Let's download a different dataset from the annotation hub.
...
Let's find H3K4me3 ChIP-seq peak files from the EpigenomeRoadMap datasets for experiment E010 (Cultured neuronal cells).
...
- There is a gtf file from Ensembl available at the URL below or in your data folder. Or you could try to fetch the gene models using **AnnotationHub**. 

"http://bifx-core3.bio.ed.ac.uk/genomes/mouse/mm10/annotation/Mus_musculus.GRCm38.78.edited.gtf"
...
txdb.mm10 <- makeTxDbFromGFF(file = "data/Mus_musculus.GRCm38.78.edited.gtf")
