...
-   Knowledge of statistical software (R, Matlab, SAS etc)
...
Biological data comes in many forms. Before performing any statistical test, we need to understand what type of data we have.

### Categorical Data

Values represent groups or labels Examples:
...
-   **Trial**: An experiment or process that can produce different outcomes
...
These events are not independent as the outcome of the first draw affects the probability of the second draw. This is an example of **conditional probability** where the probability of event B depends on the outcome of event A. We write this as p(B\|A) which means the probability of event B given that event A has occurred.

-   p(A) = p(Ace on 1st draw) = 4/52
-   p(B\|A) = p(Ace on 2nd draw \| Ace on first draw) = 3/51

These events are not independent as removing one ace from the pack changes the probability of finding another ace.
...
In the plot above, the possible outcomes (total number of heads) are on the x-axis, and the probability of each outcome is on the y-axis. The most likely outcome is 50 heads, while getting very few or very many heads is less likely. The distribution is symmetric around the mean (n\*p = 50) and has a **bell-shaped** curve.
...
A *success* is defined as the outcome of interest. It can be heads in a coin flip, patient recovery after treatment, sex determination in eggs, or any other binary outcome.
...
**Standard deviation** (𝛔 or sd) is the square root of the variance. We typically use standard deviation over variance as it can be expressed in the same units as the data, making it more interpretable.
...
A small standard deviation indicates that the data points are close to the mean, while a large standard deviation indicates that the data points are more spread out.

The mean and standard deviation determine the shape of the curve in a normal distribution. The mean determines the location of the peak, while the standard deviation determines the width of the curve. A smaller standard deviation results in a narrower curve, while a larger standard deviation results in a wider curve.
...
Although our histogram has discrete bars, the underlying distribution is continuous. The histogram is a visual representation of the distribution of values in our dataset. As we increase the number of observations, the histogram will more closely approximate the shape of the normal distribution curve. You could try increasing `n` to see this in action.

This is why sample size is important when trying to determine the underlying distribution of a dataset. The more samples we have, the more accurately we can estimate the true distribution of the data!
...
There are many types of probability distribution, with different mathematical parameters and shapes. Some of the most commonly encountered distributions in biology include:
...
The gene expression levels will be normally distributed with different means for the WT and KO groups, while the mutation counts will follow a Poisson distribution with different rates for the two groups. **Poisson distributions** are commonly used to model count data.
...
One way to describe a dataset is to list all of its values. However, this is not practical for large datasets and can be difficult to interpret.

Instead, we use **summary statistics** and **visualisation** to describe the distribution of values.
...
The `get_summary_stats()` function from the rstatix package provides a more detailed summary, including measures of central tendency (mean, median) and variability (standard deviation, interquartile range). The output is a *tibble* that can be piped into dplyr or ggplot functions.
...
-   **Median**
    -   The middle value of a dataset when the values are arranged in order. If there is an even number of values, the median is the average of the two middle values.
-   **Mode**
    -   The value that appears most frequently in a dataset. A dataset can have one mode (unimodal), more than one mode (multimodal), or no mode if all values are unique.
...
ggplot(data, aes(x = geneA_expression)) +
  geom_histogram(binwidth = 0.3, fill = "steelblue",colour="grey20") +
...
The **interquartile range (IQR)** is the difference between the third and first quartiles (IQR = Q3 - Q1) and represents the range of the middle 50% of the data.

![](images/quartiles.png)

#### Boxplot components

A boxplot, often called a box-and-whisker plot, consists of the following components:
...
-   **Whiskers**
    -   Extend to the minimum and maximum values within 1.5 times the IQR from the quartiles
-   **Outliers**
...
Boxplots are useful for comparing the distribution of values between different groups as they are easy to plot side-by-side. They can help identify differences in central tendency, variability, and the presence of outliers between groups.

**EXAMPLE: Plot geneA expression by genotype using a boxplot**
...
**EXAMPLE: Plot geneA expression against geneB expression using a scatterplot**

```{r}
p <- ggplot(data, aes(x = geneA_expression, y = geneB_expression)) +
  geom_point() +
  labs(title = "Scatterplot of geneA vs geneB",
       x = "geneA expression",
       y = "geneB expression") +
  theme_minimal()
p
...
A **regression line** can be added to the scatterplot to help visualise an overall trend in the data. Regression lines can be linear or non-linear, depending on the relationship between the variables. A linear regression line is a straight line that best fits the data points, while a non-linear regression line can take various shapes to better fit the data. Regression analysis can be used to quantify the relationship between two variables and make predictions based on that relationship.
...
We can confidently say that expression of geneA correlates with expression of geneB. When geneA is highly expressed, geneB is also highly expressed, and when geneA is lowly expressed, geneB is also lowly expressed. This suggests that there may be a relationship between the expression of these two genes.

### Correlation vs causation

Do not fall into the trap of thinking that correlation implies a causitave link between genes. Even if the two genes are strongly correlated, it does not mean that a change in expression of one is responsible for the change in the other. There could be another variable that is influencing expression of both of these genes (e.g. a common transcription factor) or the correlation could be due to chance.

Many biological processes are complex and involve multiple interacting variables, so it is important to be cautious when interpreting correlations and to consider other potential explanations for observed relationships.

![](images/causation.png)

### Other correlation methods

As well as Pearson's correlation, there are other methods for calculating correlation that can be used when the assumptions of Pearson's correlation are not met:

-   **Spearman's rank correlation**
    -   Assesses the monotonic (non-linear) relationship between two variables. It is used when the data is not normally distributed or when there are outliers.
-   **Kendall's tau**
    -   Assesses the strength of association between two variables. It is based on the number of concordant and discordant pairs of observations.
...
```{r}
## Print the ascombe dataset
anscombe

## Get summary statistics for the anscombe dataset
get_summary_stats(anscombe)
```

Notice that the mean and sd are identical for all four datasets. The correlation coefficient between x and y is also identical for all four datasets. However, when we plot the data, we can see that the relationships between x and y are very different in each dataset.
...
1.  The shape of a normal distribution is defined by the \_\_\_\_\_\_ and the \_\_\_\_\_\_\_.
2.  When a dataset has an even number of values, what is the median?
3.  The 2nd quartile is also known as the \_\_\_\_\_\_\_\_\_.
:::
